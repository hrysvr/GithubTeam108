...
public class C08_MultiDimensionalArrays {
    ...
    public static void main(String[] args) {
        ...
        int enKisaInnerArrayLenght = arr[0].length;


        for (int i = 0; i < arr.length; i++) {

            if (arr[i].length < enKisaInnerArrayLenght) {
                enKisaInnerArrayLenght = arr[i].length;
            }


        }

        // ayni index'teki elementleri topladigimizda koyacagimiz tek katli
        // bir array olusturalim
        // {{3, 4, 5}, {2, 3, 6, 7}};

        int[] toplamlarArrayi = new int[enKisaInnerArrayLenght];
        int toplam = 0;
